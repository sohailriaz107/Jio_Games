...
if (isset($_POST['single_submit']) && isset($_SESSION['usr_id']) && $_SESSION['usr_id'] != "") {
    ...
    if (get_lastBalance($user_id) < $total_point) {
        ...
        foreach ($jodi_array as $digit) {
            $amount = filter_var($_POST['jodi' . $digit], FILTER_SANITIZE_NUMBER_INT);
            if ($amount >= 5) {
...
<head>
    ...
    <title>Jodi Bidding - Jio Games</title>
    ...
    <style>
        ...
        .digit-input-box { background: #fff; border-radius: 15px; padding: 12px 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.03); border: 1px solid rgba(0,0,0,0.05); }
        ...
        <div id="content">
            ...
            <div class="container pb-4">  
				...
				if($row = $games->fetch_assoc()) {
                            ...
                            $betting_close_time = strtotime(date('Y-m-d').' '.$close_time);
                             if(in_array($day, $game_days) && time() < $betting_open_time){
							   ...
							   $bidding_status = 1; // Jodi can often be open if close is open
                             }else{
							   $bidding_status = 0;
                             }
                ...
                <div class="market-hero-card">
                    ...
                    <p class="market-date"><?php echo date('d M Y');?> • Jodi Bidding</p>
                </div>
                ...
                <form action="" method="POST" id="bidForm">
                    ...
                    <?php if($bidding_status){?>
                        ...
                        <select class="market-dropdown" name="game_id">
                            <option value="<?php echo $child_open;?>">Jodi Session</option>
                        </select>
                        ...
                        <span class="section-label">2. Tap Jodi to Place Bid</span>
                        <div class="digit-grid">
                            ...
                            foreach($jodi_array as $digit){ ?>
                                <div class="digit-input-box digit-box-clickable" data-digit="<?php echo $digit;?>">
                                    <span class="digit-label"><?php echo $digit;?></span>
                                    <input type="text" class="numeric-input" id="jodi<?php echo $digit;?>" name="jodi<?php echo $digit;?>" value="" readonly>
                                </div>
                            <?php } ?>
                        </div>
                    ...
                    <?php } else { ?>
                        <div class="closed-message-card">
                            ...
                            <p class="closed-text">Bidding for Jodi is currently closed. Please check back tomorrow!</p>
                        </div>
                    <?php } ?>
                </form>
    ...
    <script>
        ...
        document.querySelectorAll('.digit-box-clickable').forEach(box => {
            box.addEventListener('click', function() {
                ...
                const input = document.getElementById('jodi' + digit);
        ...
        function calculateTotal() {
            let total = 0;
            const jodis = <?php echo json_encode($jodi_array); ?>;
            jodis.forEach(id => {
                const val = parseInt(document.getElementById('jodi' + id).value) || 0;
                total += val;
            });
